COPY (select pd.project,pd.deployment,pd.contentpackage, 
...
group by pd.project,pd.deployment,pd.contentpackage,datasource
order by pd.project,pd.deployment,pd.contentpackage,datasource) TO STDOUT (FORMAT csv, HEADER true);
